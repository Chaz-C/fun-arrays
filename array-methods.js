...
var sumOfBankBalances = dataset.bankBalances.reduce((prev, curr, index, array) => {
...
// .filter((state) => ['WI', 'etc'].indexOf( state ) === -1 )
...
var sumOfHighInterests = dataset.bankBalances.filter((element, index, array ) => {
  switch(element.state) {
    ...
    case "DE":
      return false;
    default:
      if ( element.amount * 0.189 > 50000.00 ) {
        return true;
      }
  }
})
.reduce((prev, curr, index, array ) => {
  // console.log(curr.amount * 0.189);
    // return Math.round((prev + (parseFloat(curr.amount) * 0.189) ) * 100)/100;
    return Math.round((prev + parseFloat(curr.amount) * 0.189) * 100) / 100;
}, 0);
